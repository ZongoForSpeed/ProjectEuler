...
namespace
{
    ...
    {
        long double rayon = std::abs(1.0/k);
    ...
    long double iteration(std::map<triplet, nombre> & apollonios)
    {
        ...
        for (auto p: apollonios)
        {
            ...
            long double k4 = k1 + k2 + k3 + 2.0*std::sqrt(k1*k2 + k2*k3 + k1*k3);
            A += p.second * aire(k4);
...
ENREGISTRER_PROBLEME(199, "Iterative Circle Packing")
{
    ...
    //
    // At each iteration, a maximally sized circle is placed in each gap, which creates more gaps for 
    // the next iteration. After 3 iterations (pictured), there are 108 gaps and the fraction of the 
    // area which is not covered by circles is 0.06790342, rounded to eight decimal places.
    //
    ...
    long double A = 3.0*aire(k1);
    
    std::cout << std::fixed << std::setprecision(8);
    std::cout << "Iteration 0: " << 1.0 - A / a0 << std::endl;
    
    for (nombre n = 1; n <= 10; ++n)
    {
        A += iteration(apollonios);
        std::cout << "Iteration " << n << ": " << 1.0 - A / a0 << std::endl;
    }
