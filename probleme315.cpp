...
ENREGISTRER_PROBLEME(315, "Digital root clocks")
{
    ...
    //
    // When a clock is fed a number, it will show it and then it will start the calculation, showing 
    // all the intermediate values until it gets to the result.
    // For example, if the clock is fed the number 137, it will show: "137" → "11" → "2" and then it
    // will go black, waiting for the next number.
    //
    ...
    //
    // Sam's clock is fed e.g. number 137: the clock shows "137", then the panel is turned off, then 
    // the next number ("11") is turned on, then the panel is turned off again and finally the last 
    // number ("2") is turned on and, after some time, off.
    ...
    // Find the difference between the total number of transitions needed by Sam's clock and that needed
    // by Max's one.
