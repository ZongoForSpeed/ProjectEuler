...
namespace
{
	void solution(const vecteur & premiers, const mp_nombre limite, vecteur & resultat, mp_nombre n, fraction sigma)
	{
		...
		for (size_t a = exposant;; ++a)
		{
			...
			fraction sigma2 = sigma * fraction(mp_nombre::puissance(p, a + 1) - 1, mp_nombre::puissance(p, a) * (p - 1));
