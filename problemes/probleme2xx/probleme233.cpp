...
ENREGISTRER_PROBLEME(233, "Lattice points on a circle") {
    ...
    premiers::crible235<nombre>(limite / (puissance::puissance<nombre, unsigned>(5, 3)
                                          * puissance::puissance<nombre, unsigned>(13, 2)),
                                std::back_inserter(premiers));
    ...
    // n = q^3 * r^7
    for (nombre r: premiers4) {
        const auto r7 = puissance::puissance<nombre, unsigned>(r, 7);
        if (5 * 5 * 5 * r7 > limite)
            break;
    ...
    // n = q^2 * r^10
    for (nombre r: premiers4) {
        const auto r10 = puissance::puissance<nombre, unsigned>(r, 10);
        if (5 * 5 * r10 > limite)
            break;
