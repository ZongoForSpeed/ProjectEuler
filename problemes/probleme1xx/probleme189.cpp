...
ENREGISTRER_PROBLEME(189, "Tri-colouring a triangular grid") {
    ...
    //
    // We wish to colour the interior of each triangle with one of three colours: red, green or blue, so that no two
    // neighbouring triangles have the same colour. Such a colouring shall be called valid. Here, two triangles are said
    // to be neighbouring if they share an edge.
    // Note: if they only share a vertex, then they are not neighbours.
    //
    // For example, here is a valid colouring of the above grid:
    //
    // A colouring C' which is obtained from a colouring C by rotation or reflection is considered distinct from C
    ...
    for (auto a1: couleurs) for (auto b1: couleurs) if (b1 != a1)
    for (auto a2: couleurs) if (b1 != a2)
    for (auto b2: couleurs) if (b2 != a2)
    for (auto a3: couleurs) if (b2 != a3)
    for (auto b3: couleurs) if (b3 != a3)
    for (auto a4: couleurs) if (b3 != a4)
    ...
    for (auto g1: couleurs) if (g1 != f1) {
        ...
        triangles4.emplace_back(triangle);
    }
