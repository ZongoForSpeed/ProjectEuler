...
ENREGISTRER_PROBLEME(86, "Cuboid route") {
    ...
    //
    // It can be shown that there are exactly 2060 distinct cuboids, ignoring rotations, with integer dimensions, up to
    // a maximum size of M by M by M, for which the shortest route has integer length when M = 100. This is the least
    // value of M for which the number of solutions first exceeds two thousand; the number of solutions when M = 99 is
    // 1975.
    //
    ...
    nombre compteur = 0;
    for (nombre a = 1;; ++a) {
        for (nombre s = 2; s <= 2 * a; ++s) {
            if (polygonal::est_carre(a * a + s * s))
                compteur += (s > a + 1) ? a + 1 - (s + 1) / 2 : s / 2;
        }
