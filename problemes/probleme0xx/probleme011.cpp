...
ENREGISTRER_PROBLEME(11, "Largest product in a grid") {
    ...
    for (std::size_t i = 0; i < 20; ++i) {
        ...
        for (std::size_t j = 0; j < 20; ++j) {
            if (i + 4 < 20)
                resultat = std::max(resultat,
                                    matrice[i][j] * matrice[i + 1][j] * matrice[i + 2][j] * matrice[i + 3][j]);
            if (j + 4 < 20)
                resultat = std::max(resultat,
                                    matrice[i][j] * matrice[i][j + 1] * matrice[i][j + 2] * matrice[i][j + 3]);
            if (j + 4 < 20 && i + 4 < 20) {
                resultat = std::max(resultat, matrice[i][j] * matrice[i + 1][j + 1] * matrice[i + 2][j + 2] *
                                              matrice[i + 3][j + 3]);
                resultat = std::max(resultat, matrice[i][j + 3] * matrice[i + 1][j + 2] * matrice[i + 2][j + 1] *
                                              matrice[i + 3][j]);
            }
        }
    }
