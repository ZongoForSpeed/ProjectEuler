...
ENREGISTRER_PROBLEME(84, "Monopoly odds") {
    ...
    // from the top of the respective pile and, after following the instructions, it is returned to the bottom of the
    // pile. There are sixteen cards in each pile, but for the purpose of this problem we are only concerned with cards
    // that order a movement; any instruction not concerned with movement will be ignored and the player will remain on
    // the CC/CH square.
    // 
    ...
    // which the probability of finishing on it is zero, the CH squares will have the lowest probabilities, as 5/8
    // request a movement to another square, and it is the final square that the player finishes at on each roll that
    // we are interested in. We shall make no distinction between "Just Visiting" and being sent to JAIL, and we shall
    // also ignore the rule about requiring a double to "get out of jail", assuming that they pay to get out on their
    // next turn.
    // 
    ...
    //     E3 (3.18%) = Square 24, 
    //     and GO (3.09%) = Square 00. So these three most popular squares can be listed with the six-digit modal
    // string: 102400.
    // 
