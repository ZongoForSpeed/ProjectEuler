...
namespace {
    double T(unsigned int s, double p) {
        ...
        std::vector<double> g(s + 1, 0.0);
        std::vector<double> P(s + 1, 0.0);
        ...
        for (unsigned int i = 2; i <= s; ++i) {
            ...
            for (unsigned int k = 1; k < i; ++k) {
                ...
                g[i] = std::min(g[i], 1 + p_pos * (f[i - k] + g[k]) + (1 - p_pos) * g[i - k]);
            }
            for (unsigned int k = 1; k <= i; ++k) {
                double p_pos = 1 - P[k];
...
ENREGISTRER_PROBLEME(352, "Hexagonal orchards") {
    // Each one of the 25 sheep in a flock must be tested for a rare virus, known to affect 2% of the sheep population.
    // An accurate and extremely sensitive PCR test exists for blood samples, producing a clear positive / negative
    // result, but it is very time-consuming and expensive.
    //
    // Because of the high cost, the vet-in-charge suggests that instead of performing 25 separate tests, the following
    // procedure can be used instead:
    //
    // The sheep are split into 5 groups of 5 sheep in each group. For each group, the 5 samples are mixed together and
    // a single test is performed. Then,
    //
    //    - If the result is negative, all the sheep in that group are deemed to be virus-free.
    //    - If the result is positive, 5 additional tests will be performed (a separate test for each animal) to
    //    determine the affected individual(s).
    //
    // Since the probability of infection for any specific animal is only 0.02, the first test (on the pooled samples)
    ...
    // Consequently, all 5 groups can be screened using an average of only 1.480396016 × 5 = 7.40198008 tests, which
    // represents a huge saving of more than 70% !
    //
